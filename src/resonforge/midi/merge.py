...
def merge_midi_documents(
    inputs: list[MidiDocument],
    *,
    source_gains_db: list[float],
    normalize_velocity: int | None = None,
    use_ports: bool = False,
    output_bpm: float = 120.0,
    track_names: list[str] | None = None,
    output_beats_per_bar: int | None = None,
    bar_phase_seconds: float | None = None,
) -> tuple[MidiDocument, str]:
    """Combine in-memory MIDI documents without an intermediate file."""
    ...
    for source_index, document in enumerate(inputs):
        source = document.to_mido()
        velocity_gain = 10.0 ** (source_gains_db[source_index] / 20.0)
        output_track = mido.MidiTrack()
        name = track_names[source_index] if track_names else f"stem-{source_index + 1}"
        output_track.append(
            mido.MetaMessage("track_name", name=midi_text(name), time=0)
        )
        if use_ports:
            output_track.append(mido.MetaMessage("midi_port", port=source_index, time=0))
        tempo = 500_000
        elapsed_seconds = 0.0
        previous_tick = 0
        for message in mido.merge_tracks(source.tracks):
            ...
            if message.is_meta:
                continue
            target_tick = round(
                elapsed_seconds / (output_tempo / 1_000_000) * OUTPUT_TICKS_PER_BEAT
            )
            ...
            if copied.type == "note_on" and copied.velocity > 0:
                copied.velocity = max(1, min(127, round(copied.velocity * velocity_gain)))
    ...
    if normalize_velocity is not None and note_messages:
        scale = normalize_velocity / max(message.velocity for message in note_messages)
        for message in note_messages:
            message.velocity = max(1, min(127, round(message.velocity * scale)))
    return MidiDocument.from_mido(combined), "multi-port" if use_ports else "single-port"
